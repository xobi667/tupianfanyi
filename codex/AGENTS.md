...
## 2026-06-02 Entry: Pro Model Cost Warning

- Trigger: user discovered translation runs had used `gemini-3-pro-image-preview` and objected to the cost risk.
- What changed:
  - updated `project/README.md` with model cost and safety rules
  - updated root `DEV.md` and `codex/CURRENT_STATE.md` so future sessions see the warning first
  - recorded that the user rejected SKU/file naming-rule work for the current translation flow
- What went wrong:
  - batch translation continued without first confirming high-cost image model use
  - the work focused on completion and underweighted that image redraw calls are the expensive step
  - restored history tasks can carry old model summaries, so model choice must be checked before continuing
- Final fix:
  - do not use `gemini-3-pro-image-preview` for batch image translation unless the user explicitly confirms
  - prefer low-cost text/image models such as `gemini-3.1-flash-lite-preview` and `gemini-3.1-flash-image-preview`, or whatever cheap model the user specifies
  - do not rename files or normalize SKU naming unless the user asks again
  - keep API keys only in browser-local settings, never in docs/logs/code/screenshots/history

## 2026-06-03 Entry: Low-cost Defaults Enforcement

- Trigger: after the Pro model cost warning, continuing work needed a source-level guard instead of documentation only.
- What changed:
  - changed the app default text model to `gemini-3.1-flash-lite-preview`
  - changed the app default image model to `gemini-3.1-flash-image-preview`
  - changed legacy image-model migration so `gemini-3-pro-image-preview` maps down to the safe image default
  - changed settings placeholders and README defaults to low-cost examples
- What went wrong:
  - docs alone were not enough; the source default and migration path could still select Pro
  - the previous legacy set treated the safe image model as stale and rewrote it to the default, which was dangerous while the default was Pro
- Final fix:
  - Pro may remain only as an explicit legacy/migration warning target, not as a default, placeholder, or recommendation
  - do not resume batch translation until the current browser/app settings are confirmed low-cost or the user explicitly approves a high-cost model
